...
- `women-saree-2.html` and `women-lehenga-2.html` are identical to their `-1` counterparts — likely kept as alternates or backups
- `data.js` is loaded by `index.html` for the React SPA's product catalog
- The cart in `cart.html` reads from `localStorage` key `poshak_cart` (set by `script.js`)
- The homepage React cart uses a separate key `ps_cart_<email>` scoped per user
